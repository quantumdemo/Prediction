...
class TestStage1Docs(unittest.TestCase):
    def setUp(self):
        ...
        self.master_spec_path = os.path.join(self.docs_dir, "MASTER_SPECIFICATION.md")
        self.constitution_path = os.path.join(self.docs_dir, "ENGINEERING_CONSTITUTION.md")
    ...
    def test_files_exist(self):
        ...
        self.assertTrue(
            os.path.exists(self.constitution_path), "ENGINEERING_CONSTITUTION.md must exist"
        )
        self.assertTrue(os.path.exists(self.readme_path), "README.md must exist")

    def test_master_specification_sections(self):
        ...
        for section in required_sections:
            self.assertIn(
                section, content, f"Missing section '{section}' in MASTER_SPECIFICATION.md"
            )
    ...
    def test_requirement_ids(self):
        ...
        for prefix in req_prefixes:
            self.assertIn(prefix, content, f"Missing prefix '{prefix}' in MASTER_SPECIFICATION.md")
    ...
    def test_constitution_articles(self):
        ...
        for article in required_articles:
            self.assertIn(
                article, content, f"Missing article '{article}' in ENGINEERING_CONSTITUTION.md"
            )
